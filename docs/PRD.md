...
> **버전**: v0.5 · 2026-09-12 · **초안 — Dave 와 함께 수정 중. 코드 작업 금지, 계획만.**
...
│ 🔍 검색   ＋  │  🟠 제품_Rondo            ⋯  ⤴  ⚙ ≫    │ 봇의 작업          │
│              │ ┌────────────────────────────────────┐ │ ──────────────── │
│ 🟠 🟣 ⚪ (핀) │ │ 저장된 메시지 표시 중 · 연결 복구 시 갱신 │ │ ● 세션 · PRD 작성 │
│              │ └────────────────────────────────────┘ │   실행 중 · 12m   │
...
| 영역 | 내용 |
|---|---|
| **왼쪽 봇 목록** | 상단에 핀한 봇 아바타 행(색 = 봇 색). 아래는 활성 봇 목록 — 이름·마지막 메시지 한 줄·시각·상태 점. 확인 대기는 맨 위. 맨 아래 `＋ 후보 깨우기 (N)`. |
...
| **오른쪽 봇의 작업** (≫ 로 접힘) | ① 세션 목록과 상태 ② **폴더 파일 트리(컴팩트)** — 최근 변경 순, 봇이 만든/고친 파일에 점 ③ 루틴 ④ 하네스 요약(CLAUDE.md·스킬·MCP 개수). 이 패널은 **보조**다 — 접어도 대화만으로 일이 된다. |
...
- 목록 → 대화 두 화면. 오른쪽 패널은 **하단 시트**(위로 끌어올림), 파일 뷰어도 전체 높이 시트. 승인 카드는 대화 안 + 푸시.
- 폰에서 편집은 todo 체크·짧은 노트까지(Q8). 그 이상은 "데스크톱에서 열기" 링크.

### 4.4 봇 깨우기 — 두 갈래

1. **직접**: `＋ 후보 깨우기` → **PARA 트리 피커**(Projects·Area 의 2단계 폴더만, 하네스 있는 폴더에 ✓ 배지, 최근 수정순) → 선택 → 봇 색·아바타 자동 → 목록에 나타남.
2. **오케스트레이터에게**: *"하이드미플리즈 봇 깨워"* → 후보에 있으면 즉시 깨움, 없으면 *"폴더가 없습니다. `2. Projects/2026-09_하이드미플리즈` 로 만들고 하네스를 깔까요?"* → 승인 → 생성·깨움.

재우기·은퇴는 봇 헤더 `⋯` 또는 오케스트레이터에게.
...
- 메신저의 단순함이 우선 — 다크 기본, 봇마다 색 하나, 상태는 점. 장식 없음.
