...
2. ad-hoc 서명이라 처음엔 Gatekeeper 가 막습니다. DMG 안의 **「처음 설치 후 읽어주세요.txt」** 대로 터미널에 한 줄을 붙여 넣으면 됩니다:
   ```
...
- **폴더 선택**: PARA 트리를 접었다 펴서 아무 폴더나 고릅니다(Resources·Archive 포함). 트리에서 폴더 우클릭 → 「여기서 에이전트 시작」. 봇이 있는 폴더는 「봇 열기」.
- **메인 · 원격**: 왼쪽 아래(폰은 홈 제목 아래) 배지가 호스트 맥 자체의 창이면 「메인」, 다른 기기면 「원격 · 기기이름」. 이름은 설정 › 이름에서 바꿉니다(기본값은 컴퓨터 이름·페어링 때 고른 기기 종류).
- **할 일**: 제목을 누르면 그 자리에서 편집(⏎ 저장), 삭제는 5초 안에 되돌릴 수 있고, 눈 아이콘으로 완료 항목을 봅니다.
- 단축키: `⌘B` 목록 · `⌘⇧B` 패널 · `⌘⇧D` 문서 열 · `⌘W` 탭 닫기 · `⎋` 편집 끝.

- **레일 호버 카드** — 봇 행에 마우스를 올리면 경로·상태·세션(모델·컨텍스트)·할 일·마지막 말이 카드로 뜹니다. 접힌 스트립의 작은 플라이아웃과 같은 정보입니다.
- **할 일 행** — 글이 오른쪽 끝까지 쓰이고, 연필·맡기기·삭제는 마우스를 올리면 글 위 오른쪽에 뜹니다(폰은 항상). 긴 항목은 2줄에서 «…더», 누르면 그 행만 펼쳐집니다.
- **긴 이름** — 폴더·파일 이름은 Finder 처럼 앞뒤를 남기고 가운데를 … 로 줄입니다.
- **테마** — 설정 › 화면에서 시스템·라이트·다크를 고릅니다. 기본은 시스템을 따릅니다.
- **폰에서 할 일** — 행을 좌우로 쓸어 처리합니다. 오른쪽으로 조금=편집, 길게=완료, 왼쪽으로 조금=메뉴, 길게=삭제. 설정 › 할 일에서 네 자리를 바꿀 수 있습니다.
