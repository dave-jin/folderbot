  ...
  return <div className="col doc" style={{ flex: wide ? 3 : 1.15 }}>
    ...
    {rel ? <div className="dtb"><span>{dir}</span><span>/</span><span className="nm">{name}</span>
      <span className="r">
        ...
        {/* 🔴 **모드 전환은 눈·연필 두 아이콘뿐.** «편집 중» 을 크게 알리지 않는다 — 원래 고칠 수 있는 것이
            기본이기 때문이다(승인된 「A · 문서처럼」 캔버스). 상태는 «자동 저장» 한 마디로 조용히 둔다. */}
        {edit ? <span className={`st ${saveSt === 'fail' ? 'err' : ''}`}>{saveSt === 'saving' ? '저장 중' : saveSt === 'fail' ? '저장 실패' : '자동 저장'}</span> : null}
        {doc?.kind === 'text' ? <button className={`ib ${edit ? 'on' : ''}`} onClick={() => (edit ? finishEdit() : startEdit())} title={edit ? '읽기 (⎋)' : '편집 — 자동 저장'}><Icon n={edit ? 'eye' : 'edit'} size={13} /></button> : null}
